...
Status: GPU-validated redesign after the GraCE/CUDA investigation, source audit, H200 Nsight
Compute profile, exact-shape GEMV probes, and strong-baseline replay. This document is normative
with:
...
MegaBake V2.3 is a **traffic-first, bucket- and target-specialized one-grid inference compiler**.
Its research north star is to emit one owned GPU compute grid that beats the strongest equivalent
`torch.compile` mode for each supported target/model/bucket. “One grid” describes an invocation,
not one universal binary or launch envelope shared by all workloads.
...
V2.3 retains the V2.2 contracts and adds five GPU-derived corrections:
...
EntryLaunchEnvelope
  threads_per_cta
  warp_role_topology
  register_and_stack_ceiling
  static_and_dynamic_smem
...
- detect MIG memory/SM/L2 fractions, calibrate bandwidth, and remove name-based peak heuristics;
...
- implement warp-reduction GEMV first, then compare CTA/split-K, padded-M tensor-core, and one
  MPK/CuTe candidate;
...
general scheduler. It is a small compiler that emits one resource-compatible grid per supported
bucket, builds excellent shape/precision-specific device bodies, recomputes cheap values when that
avoids communication, and uses CUDA Graphs as the measured fallback for incompatible or vendor
boundaries.
